...
function getBytes(transaction, skipSignature, skipSecondSignature) {
	...
	switch (transaction.type) {
		...
		case 6: // register
			var idBuff = Buffer.from(transaction.asset.id, "hex");
			var typeBuff = Buffer.from([transaction.asset.type]);
			var dataBuf = Buffer.from(transaction.asset.data, "utf8");
			var formatBuf = Buffer.from(transaction.asset.format, "utf8");
			var dataSigBuf = Buffer.from(transaction.asset.dataSig, "hex");

			assetBytes = Buffer.concat([idBuff, typeBuff, dataBuf, formatBuf, dataSigBuf], idBuff.length + typeBuff.length + dataBuf.length + formatBuf.length + dataSigBuf.length);
			assetSize = assetBytes.length;
			break;

		case 7: // verify
			var dataBuf = Buffer.from(transaction.asset.data, "utf8");
			var signatureBuf = Buffer.from(transaction.asset.signature, "utf8");

			assetBytes = Buffer.concat([dataBuf, signatureBuf], dataBuf.length + signatureBuf.length);
			assetSize = assetBytes.length;
			break;
	}
 ...
 * @returns {ECSignature}
 */
function signBuffer(buff, keys) {
	if(!buff || !keys)
		return;

	var hash = crypto.createHash("sha256").update(buff).digest();
