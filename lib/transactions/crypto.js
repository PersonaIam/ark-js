...
function getBytes(transaction, skipSignature, skipSecondSignature) {
	...
	switch (transaction.type) {
		...
		case 6: // register
			var idBuff = Buffer.from(transaction.asset.id, "utf8");
			...
			assetBytes = Buffer.concat([idBuff, typeBuff, dataBuf, dataSigBuf], idBuff.length + typeBuff.length + dataBuf.length + dataSigBuf.length);
			assetSize = assetBytes.length;
			break;
