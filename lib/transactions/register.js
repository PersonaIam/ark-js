...
function createRegistration(secret, type, data, secondSecret) {
  ...
  var owner = crypto.getAddress(keys.publicKey);

  var typeBuff = Buffer.from([type]);
  var ownerBuff = Buffer.from(owner, "utf8");
  var dataBuff = Buffer.from(data, "utf8");
  var id = Buffer.concat([ownerBuff, typeBuff, dataBuff], ownerBuff.length + typeBuff.length + dataBuff.length);

  id = crypto.hashBuffer("sha256").update(id).digest();
